...
@Component({
    ...
    template: `
        <div class="flex flex-col gap-4" [dir]="layoutService.layoutConfig().lang === 'ar' ? 'rtl' : 'ltr'">
            
            <div class="flex flex-col gap-2">
                <span class="text-sm text-muted-color font-semibold text-start block">Primary</span>
                <div class="pt-2 flex gap-2 flex-wrap justify-start">
                    @for (primaryColor of primaryColors(); track primaryColor.name) {
                        <button
                            ...
                            class="cursor-pointer w-5 h-5 rounded-full flex shrink-0 items-center justify-center outline-offset-1 shadow transition-all hover:scale-110 active:scale-90"
                            [style]="{
            ...
            <div class="flex flex-col gap-2">
                <span class="text-sm text-muted-color font-semibold text-start block">Surface</span>
                <div class="pt-2 flex gap-2 flex-wrap justify-start">
                    @for (surface of surfaces; track surface.name) {
                        <button
                            ...
                            class="cursor-pointer w-5 h-5 rounded-full flex shrink-0 items-center justify-center p-0 outline-offset-1 transition-all hover:scale-110"
                            [ngClass]="{
            ...
            <div class="flex flex-col gap-2">
                <span class="text-sm text-muted-color font-semibold text-start block">Presets</span>
                <p-selectbutton [options]="presets" [ngModel]="selectedPreset()" (ngModelChange)="onPresetChange($event)" [allowEmpty]="false" size="small" class="w-full" />
            </div>

            <div *ngIf="showMenuModeButton()" class="flex flex-col gap-2">
                <span class="text-sm text-muted-color font-semibold text-start block">Menu Mode</span>
                <p-selectbutton [ngModel]="menuMode()" (ngModelChange)="onMenuModeChange($event)" [options]="menuModeOptions" [allowEmpty]="false" size="small" class="w-full" />
            </div>
        </div>
    `,
    host: {
        '[class]': 'hostClasses()',
        '[style.direction]': "layoutService.layoutConfig().lang === 'ar' ? 'rtl' : 'ltr'"
    }
})
export class AppConfigurator implements OnInit {
    ...
    // إدارة كلاسات الظهور بناءً على اتجاه الصفحة
    hostClasses = computed(() => {
        const isRtl = this.layoutService.layoutConfig().lang === 'ar';
        const base = 'hidden absolute top-13 w-72 p-4 bg-surface-0 dark:bg-surface-900 border border-surface rounded-border origin-top shadow-xl z-50';
        // في RTL تظهر القائمة من اليسار لتبقى داخل الشاشة، وفي LTR من اليمين
        return isRtl ? `${base} left-0 right-auto` : `${base} right-0 left-auto`;
    });
    ...
    getPresetExt() {
        ...
        if (color.name === 'noir') {
            ...
            const commonSemantic = { primary: color.palette };
            if (preset === 'Nora') {
                return {
                    semantic: {
                        ...commonSemantic,
