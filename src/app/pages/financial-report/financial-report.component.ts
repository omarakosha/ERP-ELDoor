...
filterReport() {
  ...
  this.filteredTransactions = this.transactions.filter(t => {
    const matchMainAccount = !this.filterData.mainAccountId || t.mainAccountId === this.filterData.mainAccountId;
    const matchSubAccount = !this.filterData.subAccountId || t.subAccountId === this.filterData.subAccountId;
    ...
    // ✅ إذا لم يُحدد أي فلتر للحساب أو مركز التكلفة، نعتبر كل شيء صالح
    return matchMainAccount && matchSubAccount && matchCostCenter && matchDate && matchType && matchProject && matchCurrency;
  });
...
selectParentAccount(account: Account) {
    if (this.currentFocus === 'mainAccount') {
      this.filterData.mainAccount = account.name; 
      this.filterData.mainAccountId = account.id; 
    }
    else if (this.currentFocus === 'subAccount') {
      this.filterData.accountName = account.name; 
      this.filterData.accountNumber = account.id.toString();

      // ✅ إضافة هذا السطر لحفظ id الحساب التحليلي
      this.filterData.subAccountId = account.id;
      this.selectedSubAccount = account; // لتطبيق الفلترة لاحقًا
    }
